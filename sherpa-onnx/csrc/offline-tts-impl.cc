...
#include "sherpa-onnx/csrc/offline-tts-vits-generic-impl.h"
...
std::unique_ptr<OfflineTtsImpl> OfflineTtsImpl::Create(
  ...
  if (config.model.provider == "axera") {
    ...
    if (!config.model.vits.model.empty()) {
      return std::make_unique<
          OfflineTtsVitsGenericImpl<OfflineTtsVitsModelAxera>>(config);
    } else if (!config.model.kokoro.model.empty()) {
      return std::make_unique<
          OfflineTtsKokoroAxeraImpl<OfflineTtsKokoroModelAxera>>(config);
    } else {
      SHERPA_ONNX_LOGE(
          "Only VITS/Melo and Kokoro models are currently supported by axera "
          "for "
          "non-streaming TTS.");
      SHERPA_ONNX_EXIT(-1);
      return nullptr;
  ...
  if (config.model.provider == "axcl") {
    ...
    if (!config.model.vits.model.empty()) {
      return std::make_unique<
          OfflineTtsVitsGenericImpl<OfflineTtsVitsModelAxcl>>(config);
    } else if (!config.model.kokoro.model.empty()) {
      return std::make_unique<
          OfflineTtsKokoroAxclImpl<OfflineTtsKokoroModelAxcl>>(config);
    } else {
      SHERPA_ONNX_LOGE(
          "Only VITS/Melo and Kokoro models are currently supported by axcl "
          "for "
          "non-streaming TTS.");
      SHERPA_ONNX_EXIT(-1);
      return nullptr;
...
std::unique_ptr<OfflineTtsImpl> OfflineTtsImpl::Create(
  ...
  if (config.model.provider == "axera") {
    ...
    if (!config.model.vits.model.empty()) {
      return std::make_unique<
          OfflineTtsVitsGenericImpl<OfflineTtsVitsModelAxera>>(mgr, config);
    } else if (!config.model.kokoro.model.empty()) {
      return std::make_unique<
  ...
  if (config.model.provider == "axcl") {
    ...
    if (!config.model.vits.model.empty()) {
      return std::make_unique<
          OfflineTtsVitsGenericImpl<OfflineTtsVitsModelAxcl>>(mgr, config);
    } else if (!config.model.kokoro.model.empty()) {
      return std::make_unique<
