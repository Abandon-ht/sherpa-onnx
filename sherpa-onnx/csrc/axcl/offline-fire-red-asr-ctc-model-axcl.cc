...
OfflineFireRedAsrCtcModelAxcl::OfflineFireRedAsrCtcModelAxcl(
...
OfflineFireRedAsrCtcModelAxcl::~OfflineFireRedAsrCtcModelAxcl() = default;

std::vector<Ort::Value> OfflineFireRedAsrCtcModelAxcl::Forward(
    Ort::Value features, Ort::Value features_length) {
  return impl_->Forward(std::move(features), std::move(features_length));
}

int32_t OfflineFireRedAsrCtcModelAxcl::VocabSize() const {
  return impl_->VocabSize();
}

int32_t OfflineFireRedAsrCtcModelAxcl::SubsamplingFactor() const {
  return impl_->SubsamplingFactor();
}

OrtAllocator *OfflineFireRedAsrCtcModelAxcl::Allocator() const {
  return impl_->Allocator();
}

void OfflineFireRedAsrCtcModelAxcl::NormalizeFeatures(float *features,
                                                      int32_t num_frames,
                                                      int32_t feat_dim) const {
...
#if __ANDROID_API__ >= 9
template OfflineFireRedAsrCtcModelAxcl::OfflineFireRedAsrCtcModelAxcl(
