...
struct FireRedVadModelConfig {
  ...
  // Frame shift in samples. Default is 160 (10ms at 16kHz)
  int32_t window_size = 160;
  ...
  // If left as a negative value, the default rule applies:
  //     neg_threshold = max(threshold - 0.15f, 0.01f)
  // This prevents the exit threshold from becoming negative when threshold <
  // 0.15.
